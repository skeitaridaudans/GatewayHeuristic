...
{
	double deltaX = goal.x > start.x ? goal.x - start.x : start.x - goal.x;
	double deltaY = goal.y > start.y ? goal.y - start.y : start.y - goal.y;
	if(deltaX > deltaY) {
		return deltaX + 0.4 * deltaY;
	}
	else {
		return deltaY + 0.4 * deltaX;
	}
}
